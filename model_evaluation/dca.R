    ...
    if(intervention==TRUE) {
      ...
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          ...
          lines(nb$threshold,data.matrix(nb[predictors[m]]),col='#E64B35B2',lty=1,lwd=1.5)
        }
        # adding each model to the legend
        legendlabel <- c(legendlabel, 'Radiomics')
        legendcolor <- c(legendcolor, '#E64B32B2')
        legendwidth <- c(legendwidth, 2)
        legendpattern <- c(legendpattern, 2)
      }
    }
    # then add the legend
    legend("topright", legendlabel, cex=0.6, col=legendcolor, lwd=legendwidth, lty=legendpattern)
